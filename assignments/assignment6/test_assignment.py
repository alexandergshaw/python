...
class StudentCodeTests(unittest.TestCase):
    ...
    def test_parse_numbers_skips_bad_lines(self):
        self.assertEqual(self.module.parse_numbers(["10", "x", "20", ""]), [10, 20])

    def test_parse_numbers_all_good(self):
        self.assertEqual(self.module.parse_numbers(["5"]), [5])
        self.assertEqual(self.module.parse_numbers(["1", "2", "3"]), [1, 2, 3])

    def test_parse_numbers_empty(self):
        self.assertEqual(self.module.parse_numbers([]), [])

    def test_parse_numbers_keeps_order(self):
        self.assertEqual(self.module.parse_numbers(["3", "oops", "1", "2"]), [3, 1, 2])
